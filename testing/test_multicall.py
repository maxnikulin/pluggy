...
def test_hookwrapper_yield_not_executed():
    @hookimpl(hookwrapper=True)
    def m1():
        if False:
            yield
...
@pytest.mark.xfail
def test_hookwrapper_blocked_generator_exit():
    out = []
...
def test_unwind_inner_wrapper_teardown_exc():
    out = []
...
def test_suppress_inner_wrapper_teardown_exc():
    out = []
